-- Migration: Create Travel & Trip Planner Tables (Fixed)
-- Description: Adds tables for travel planning with fixed foreign key constraints
-- Date: 2025-01-28
...
CREATE TABLE TripExpenses (
    ...
    FOREIGN KEY (itineraryItemId) REFERENCES ItineraryItems(id) ON DELETE NO ACTION
);
...
CREATE TABLE TripSharedExpenses (
    ...
    paidAt DATETIME2,
    createdAt DATETIME2 DEFAULT GETDATE(),
    FOREIGN KEY (expenseId) REFERENCES TripExpenses(id) ON DELETE NO ACTION,
    FOREIGN KEY (companionId) REFERENCES TripCompanions(id) ON DELETE NO ACTION
);
